...
class Input {
  ...
  static initialize(element) {
    if (
      ...
      element.hasAttribute("data-max-rows")
    ) {
    ...
    if (
      (element.getAttribute("type") === "text" ||
        element.tagName.toLowerCase() === "textarea") &&
      (element.dataset.wordLengthWarning || element.dataset.wordLengthError)
    ) {
...
const setTextMaxWordLength = (element) => {
  ...
  element.addEventListener("input", (event) => {
    ...
    warningSpan.dataset.visible = words.some(
      (word) => word.length > wordLengthWarning
    );
...
const initializeQuantityField = (element) => {
  ...
  input.addEventListener("change", (e) => {
    if (
      (input.value !== "" && input.hasAttribute("data-empty")) ||
      input.hasAttribute("data-scrubber")
    ) {
...
function initializeRangeSlider(element) {
  ...
  element.style.setProperty(
    "--value",
    isRTL ? 100 - element.value : element.value
  );
